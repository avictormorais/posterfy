...
`,Hl=({onRecreate:e})=>{const{t}=B(),[o,n]=d.useState(null),[a,s]=d.useState(!1),[l,p]=d.useState(!1),[c,h]=d.useState(null),[m,v]=d.useState(null);d.useEffect(()=>{const b=()=>{s(window.innerWidth<=770),p(window.innerWidth>480&&window.innerWidth<=1200)};return b(),window.addEventListener("resize",b),()=>window.removeEventListener("resize",b)},[]);const T=(b,w)=>{h(b),v(w),document.body.style.overflow="hidden"},g=()=>{h(null),document.body.style.overflow="auto"},f=()=>{h(null),e(m),document.body.style.overflow="auto"};return r.jsxs(r.Fragment,{children:[r.jsx(Ml,{children:zl.map((b,w)=>r.jsx(Al,{index:w,isHovered:o===w,hoveredIndex:o,otherIsHovered:o!==null,isMobile:a,isTablet:l,onMouseEnter:()=>n(w),onMouseLeave:()=>n(null),onClick:()=>T(b.coverImage||"/placeholder.svg",b.JSONConfig),children:r.jsx("img",{src:b.coverImage||"/placeholder.svg",alt:`${b.artist} - ${b.title}`})},b.id))}),c&&r.jsx(Ll,{onClick:g,children:r.jsxs(Pl,{onClick:b=>b.stopPropagation(),children:[r.jsxs(Ol,{children:[r.jsx($l,{onClick:g,children:"×"}),r.jsx(Dl,{onClick:f,children:t("RecreatePoster")})]}),r.jsx("img",{src:c,alt:"Album cover"})]})})]})},Fl=u.div`
  display: flex;
  width: 100%;
  justify-content: center;
  align-items: center;
...
`;function v1({DefaultColor:e,image:t,predefinedColors:o,position:n,onDone:a,onClose:s}){const[l,p]=d.useState(e),[c,h]=d.useState(!1),[m,v]=d.useState(null),T=d.useRef(null),g=d.useRef(null);function f(){h(!c)}function b(x){p(x.target.value===""?"#":x.target.value)}function w(x){const S=T.current,j=g.current.getBoundingClientRect(),_=S.getContext("2d"),R=Math.floor(x.clientX-j.left),z=Math.floor(x.clientY-j.top),M=`#${[..._.getImageData(R,z,1,1).data].slice(0,3).map(A=>A.toString(16).padStart(2,"0")).join("")}`;p(M),f()}function y(x){const S=T.current,j=g.current.getBoundingClientRect(),_=Math.floor(x.clientX-j.left),R=Math.floor(x.clientY-j.top);if(S){const z=S.getContext("2d");try{const M=`#${[...z.getImageData(_,R,1,1).data].slice(0,3).map(A=>A.toString(16).padStart(2,"0")).join("")}`;v(M)}catch{v(null)}}}function C(){v(null)}return r.jsxs(l1,{style:{position:"absolute",top:n.top,left:n.left,zIndex:1e3},children:[r.jsx("canvas",{ref:T,style:{display:"none"}}),c?r.jsx(x1,{ref:g,crossOrigin:"anonymous",draggable:"false",src:t,onClick:w,onMouseMove:y,onMouseLeave:C,onLoad:()=>{const x=T.current,S=x.getContext("2d"),I=g.current;x.width=I.width,x.height=I.height,S.drawImage(I,0,0,I.width,I.height)},style:m?{borderColor:m}:{}}):r.jsx(c1,{color:l,onChange:p}),r.jsx(jt,{children:o.map(x=>r.jsx(d1,{style:{backgroundColor:x},onClick:()=>p(x)},x))}),r.jsxs(jt,{children:[r.jsx(u1,{children:"Hex"}),r.jsx(p1,{value:l,onChange:b})]}),r.jsxs(jt,{children:[c?r.jsx(f1,{onClick:f}):r.jsx(h1,{onClick:f}),r.jsx(m1,{onClick:s}),r.jsx(g1,{onClick:()=>{a(l),Ui(l)}})]})]})}const b1=u.div`
    display: flex;
    flex-direction: column;
    margin: 10px;
`,y1=u.p`
...
`,Bd=u.div`
    ...
    text-align: center;
    margin-top: 10px;
`;function Ud({isOpen:e,onClose:t,onProfileUpdate:o}){const{user:n,updateUser:a}=ze(),{t:s}=B(),[l,p]=d.useState({name:"",username:""}),[c,h]=d.useState({}),[m,v]=d.useState(!1),[T,g]=d.useState(""),[f,b]=d.useState(!1);d.useEffect(()=>{n&&p({name:n.name||"",username:n.username||""})},[n]);const w=()=>{const S={};return l.name.trim()||(S.name=s("NameIsRequired")),l.username.trim()?l.username.length<3?S.username=s("UsernameMinLength"):/^[a-zA-Z0-9]+$/.test(l.username)||(S.username=s("UsernameFormat")):S.username=s("UsernameIsRequired"),h(S),Object.keys(S).length===0},y=S=>{const{name:I,value:j}=S.target;p(_=>({..._,[I]:j})),c[I]&&h(_=>({..._,[I]:""})),T&&g("")},C=async S=>{var I,j,_,R;if(S.preventDefault(),!!w()){v(!0),g("");try{await J.updateUserProfile({name:l.name.trim(),username:l.username.trim()}),a({...n,name:l.name.trim(),username:l.username.trim()}),g(s("ProfileUpdatedSuccessfully")),o&&o(),setTimeout(()=>{x()},1500)}catch(z){console.error("Profile update failed:",z),(I=z.message)!=null&&I.includes("409")||(j=z.message)!=null&&j.includes("taken")?h({username:s("UsernameTaken")}):(_=z.message)!=null&&_.includes("400")||(R=z.message)!=null&&R.includes("invalid")?h({username:s("InvalidUsernameFormat")}):h({general:s("FailedToUpdateProfile")})}finally{v(!1)}}},x=()=>{b(!0),setTimeout(()=>{b(!1),t(),h({}),g("")},300)};return e?(document.body.style.overflow="hidden",r.jsx(Rd,{isClosing:f,children:r.jsxs(Ed,{isClosing:f,children:[r.jsxs(zd,{children:[r.jsx(Pd,{children:r.jsx(ie,{width:25,height:25,fill:"var(--textColor)"})}),r.jsx(Md,{children:s("EditProfile")}),r.jsx(Ld,{onClick:x,children:r.jsx(Ad,{})})]}),r.jsx(Od,{}),r.jsxs($d,{onSubmit:C,children:[r.jsxs(po,{children:[r.jsx(ho,{htmlFor:"name",children:s("Name")}),r.jsx(fo,{type:"text",id:"name",name:"name",value:l.name,onChange:y,placeholder:s("EnterYourName"),disabled:m}),c.name&&r.jsx(_t,{children:c.name})]}),r.jsxs(po,{children:[r.jsx(ho,{htmlFor:"username",children:s("Username")}),r.jsx(fo,{type:"text",id:"username",name:"username",value:l.username,onChange:y,placeholder:s("EnterYourUsername"),disabled:m}),c.username&&r.jsx(_t,{children:c.username})]}),c.general&&r.jsx(_t,{children:c.general}),T&&r.jsx(Bd,{children:T}),r.jsxs(Dd,{children:[r.jsx(Hd,{type:"button",onClick:x,disabled:m,children:s("Cancel")}),r.jsx(Fd,{type:"submit",disabled:m,children:s(m?"Saving":"SaveChanges")})]})]})]})})):null}const Nd=u.div`
    display: flex;
    align-items: center;
    min-height: 85dvh;
    flex-direction: column;
