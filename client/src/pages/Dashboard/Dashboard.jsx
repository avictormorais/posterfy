...
export default function Dashboard() {
    ...
    }, [isAuthenticated, pinnedPosterId]);
    ...
    if (loading) return <Loading isVisible={true} initialFade={true} />;
    if (!user)   return null;

    const displayName = userProfile?.name || user.name || '';

    // Computed
    const filteredMyPosters = visibilityFilter === 'all'
        ? myPosters
                    ...
                    myLoading && !myFetched ? (
                        ...
                        <>
                            ...
                            {filteredMyPosters.length === 0 ? (
                                <EmptyState><EmptyText>{t('DASH_NoPosters')}</EmptyText></EmptyState>
                            ) : (
                                <PosterGrid>
                            ...
                            {myHasMore && (
                                <LoadMoreBtn
                                    onClick={() => fetchMyPosters(myPage + 1, true)}
                                    disabled={myLoading}
                                >
